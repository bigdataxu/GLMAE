...
class Logger(object):
    ...
    def print_statistics(self, run=None, f=sys.stdout, last_best=False):
        if run is not None:
            ...
            r1 = best_result[:, 0]
            print(f'Highest Valid: {r1.mean():.2f} ± {r1.std():.2f}', file=f)
            r2 = best_result[:, 1]
            print(f'   Final Test: {r2.mean():.2f} ± {r2.std():.2f}', file=f)
            if self.info.writer:
                fh = open('./log/{}_{}.txt'.format(self.info.dataset, self.info.layer), 'a')
                fh.write(f'Highest Valid: {r1.mean():.2f} ± {r1.std():.2f}'
                         f'   Final Test: {r2.mean():.2f} ± {r2.std():.2f}')
                fh.write('\r\n')
                fh.flush()
                fh.close()
