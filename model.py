...
# custom modules
from loss import info_nce_loss, ce_loss, log_rank_loss, hinge_auc_loss, auc_loss,huxg_loss
...
class GNNEncoder(nn.Module):
    def __init__(
        ...
        for i in range(num_layers):
            ...
            heads = 1 if 'gat' not in layer else 4
...
class LPDecoder_ogb(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels=256, out_channels=1, encoder_layer=2, num_layers=3,
                 dropout=0.05, activation='relu',layer="gcn"):
        super(LPDecoder_ogb, self).__init__()
        n_layer = encoder_layer * encoder_layer
        in_channels = in_channels if 'gat' not in layer else in_channels*4
        self.mlps = torch.nn.ModuleList()
        self.mlps.append(torch.nn.Linear(in_channels * n_layer, hidden_channels))
        for _ in range(num_layers - 2):
            self.mlps.append(torch.nn.Linear(hidden_channels, hidden_channels))
        self.mlps.append(torch.nn.Linear(hidden_channels, out_channels))
    ...
    def forward(self, h, edge,sigmoid=True):
...
class DegreeDecoder(nn.Module):
    ...
    def __init__(
        self, in_channels, hidden_channels, out_channels=1,encoder_layer = 2,
        num_layers=2, dropout=0.5, activation='relu',layer="gcn"
    ):

        super().__init__()
        self.mlps = nn.ModuleList()
        in_channels = in_channels if 'gat' not in layer else in_channels * 4
        in_channels = in_channels * encoder_layer
...
class MaskGAE(nn.Module):
    ...
    def train_epoch(
        self, data, optimizer, alpha=2,
        batch_size=2 ** 16, grad_norm=1.0,denominator = 10
    ):
        ...
        alpha = float(Fraction(alpha, pow(denominator, 3)))
        for perm in DataLoader(
            range(masked_edges.size(1)), batch_size=batch_size, shuffle=True,num_workers=4
        ):
            ...
            if self.degree_decoder is not None:
                ...
                loss = (1-alpha)*loss + alpha * F.mse_loss(self.degree_decoder(z, remaining_edges).squeeze(), deg)
            loss.backward()
            if grad_norm > 0:
    ...
    def batch_predict(self, z, edges, batch_size=2 ** 16,):
        preds = []
        for perm in DataLoader(range(edges.size(1)), batch_size,num_workers=4):
            edge = edges[:, perm]
    ...
    def test(self, z, pos_edge_index, neg_edge_index, batch_size=2**16):
        ...
        pos_pred = self.batch_predict(z, pos_edge_index,batch_size = batch_size)
        neg_pred = self.batch_predict(z, neg_edge_index,batch_size = batch_size)
    ...
    def test_ogb(self, z_train, z_full, splits, evaluator, batch_size=2**16):
        ...
        pos_valid_pred = self.batch_predict(z_train, pos_valid_edge,batch_size = batch_size)
        neg_valid_pred = self.batch_predict(z_train, neg_valid_edge,batch_size = batch_size)
